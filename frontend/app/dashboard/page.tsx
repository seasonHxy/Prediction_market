      ...
      <main className="container mx-auto px-4 py-12">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div>
              ...
              <div className="space-y-4">
                {activePositions.map((position) => {
                  const daysLeft = Math.ceil((position.endsAt.getTime() - Date.now()) / (24 * 60 * 60 * 1000))
                  return (
                    <Card
                      key={position.id}
                    ...
                    >
                      <div className="mb-4">
                        <h3 className="font-semibold text-foreground mb-2">{position.market}</h3>
                        <Badge variant={position.side === "YES" ? "secondary" : "outline"}>
                      ...
                      <div className="grid grid-cols-4 gap-4 mb-4 text-sm">
                        <div>
                          ...
                          <p className="font-semibold text-foreground">{position.amount} USDC</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground mb-1">Current Prob</p>
                          <p className="font-semibold text-foreground">{position.currentProb}%</p>
                        </div>
                        <div>
                        ...
                        <div className="text-right">
                          ...
                          <p className="font-semibold text-green-500">+{(position.amount * 1.2).toFixed(0)} USDC</p>
                        </div>
                      </div>
                      <Button
                        variant="outline"
                  ...
                  <Card key={market.id} className="p-6 border border-border bg-card">
                    <div className="mb-4">
                      <div className="flex justify-between items-start">
                        ...
                        <Badge variant={market.result === "WON" ? "secondary" : "destructive"}>{market.result}</Badge>
                      </div>
                    </div>
                    <div className="grid grid-cols-4 gap-4 text-sm">
                      <div>
                        <p className="text-muted-foreground mb-1">Your Stake</p>
                        <p className="font-semibold text-foreground">{market.amount} USDC</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p
                          className={`font-semibold ${market.payout > 0 ? "text-green-500" : "text-muted-foreground"}`}
                        >
          ...
          <div className="space-y-6">
            ...
            {/* AI Insights */}
            <Card className="p-6 border border-border bg-card bg-gradient-to-br from-primary/5 to-accent/5">
              ...
              <div className="space-y-3 text-sm">
                <div className="p-3 rounded-lg bg-muted/50">
                  ...
                  <p className="text-muted-foreground">3 high-confidence markets added today</p>
                </div>
                <div className="p-3 rounded-lg bg-muted/50">
                  <p className="font-medium text-foreground mb-1">Your Forecast Accuracy</p>
                  <p className="text-muted-foreground">Better than 72% of forecasters</p>
                </div>
              </div>
            </Card>
          </div>
